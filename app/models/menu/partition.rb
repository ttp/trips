class Menu::Partition < ActiveRecord::Base
  ...
  def porter(porter_id)
    @porters ||= partition_porters.index_by(&:id)
    @porters[porter_id]
  end

  def porters_count
    @porters_count ||= partition_porters.count
  end

  def porter_products_by_entity(entity)
    @porter_product_entities ||= porter_products.to_a.group_by {|entity| entity.day_entity_id }
